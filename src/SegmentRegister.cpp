...
void SegmentRegister::SetCache(const GdtGate& gdt_gate){
    ...
    this->cache.limit      = 0x000FFFFF & ((((uint32_t)gdt_gate.limit_high)<<16) | (uint32_t)gdt_gate.limit_low);
...
void SegmentRegister::Set(const uint16_t data, function<GdtGate*(uint16_t)> GetGdtGate, function<GdtGate*(uint16_t)> GetLdtGate, bool protected_mode){
    if(protected_mode){
        this->selector.raw = data;
        GdtGate* gdt_gate;
