...
void Fdc::ProcessCommand(uint8_t command) {
  switch (command) {
    default:
      this->obj->Error(
          "Not implemented: command = 0x%02X at Fdc::ProcessCommand", command);
      break;
  }
}
...
Fdc::FDC_COMMAND_TYPE Fdc::GetFdcCommandType(uint8_t data) {
  switch (data) {
    ...
    default:
      this->obj->Error(
          "Not implemented: fdc_command_type=%02X at Fdc::GetFdcCommandType",
          data);
  }
}
  ...
  switch (this->fdc_mode) {
    ...
    case FDC_COMMAND_MODE:
      switch (this->fdc_command_type) {
        ...
        default:
          this->obj->Error(
              "Not implemented: fdc_command_tyep=%d at "
              "Fdc::ProcessCommandForOut8",
              this->fdc_command_type);
      }
      break;
    case FDC_EXECUTION_MODE:
      switch (this->fdc_command_type) {
        ...
        case FDC_COMMAND_WRITE_SECTOR:
          // TODO : マジックナンバーを修正する。
          this->buff[this->head * 18 * SECTOR_SIZE +
                     this->track * 18 * 2 * SECTOR_SIZE +
                     (this->sector - 1) * SECTOR_SIZE +
                     count_in_fdc_in_execution_write_sector] = data;
        ...
        default:
          this->obj->Error(
              "Not implemented: fdc_command_type=%d at "
              "Fdc::ProcessCommandForIn8",
              this->fdc_command_type);
      }
      break;
    default:
      this->obj->Error(
          "Not implemented: fdc_mode=%d Fdc::ProcessCommandForOut8",
          this->fdc_mode);
  }
}
  ...
  switch (this->fdc_mode) {
    ...
    case FDC_EXECUTION_MODE:
      switch (this->fdc_command_type) {
        ...
        case FDC_COMMAND_READ_SECTOR:
          data = this->buff[this->head * 18 * SECTOR_SIZE +
                            this->track * 18 * 2 * SECTOR_SIZE +
                            (this->sector - 1) * SECTOR_SIZE +
    ...
    default:
      this->obj->Error(
          "Not implemented: fdc_cmode=%d at Fdc::ProcessCommandForIn8",
          this->fdc_mode);
  }
  return data;
}
